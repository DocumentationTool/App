...
public class UserFunctions implements IDBFunctions, UserCalls, GroupCalls {
    ...
    public boolean addUser(RepoId repoId, UserProfile user) throws CoreSqlException {
        ...
        try {
            ...
            try (var statement = connection.prepareStatement("INSERT INTO UserPermissions(user_id, path, type) VALUES(?,?,?)")) {
                for (Permission<UserId> permission : user.getPermissions().values()) {
            ...
            for (GroupId groupId : user.getGroups()) {
                addUserToGroup(connection, repoId, groupId, user.getId());
            }
    ...
    @Override
    public void addRole(RepoId repoId, UserId userId, Role role) {
        log.info("Adding role '{}' to user '{}' in repo '{}'", role, userId, repoId.id());
        Connection connection = database.getConnection();
        try (var statement = connection.prepareStatement("INSERT INTO UserRoles(user_id, role_id) VALUES(?,?)")) {
            statement.setString(1, userId.id());
            statement.setString(2, role.name());
            ...
            log.info("Role '{}' added to user '{}' in repo '{}'", role, userId, repoId.id());
        } catch (SQLException e) {
    ...
    @Override
    public void removeRole(RepoId repoId, UserId userId, Role role) {
        log.info("Removing role '{}' from user '{}' in repo '{}'", role, userId, repoId.id());
        Connection connection = database.getConnection();
        try (var statement = connection.prepareStatement("DELETE FROM UserRoles WHERE user_id = ? AND role_id = ?")) {
            ...
            log.info("Role '{}' removed from user '{}' in repo '{}'", role, userId, repoId.id());
        } catch (SQLException e) {
    ...
    public boolean addUserToGroup(RepoId repoId, GroupId groupId, UserId userId) throws CoreException {
        ...
        Connection connection = database.getConnection();
        try {
            return addUserToGroup(connection, repoId, groupId, userId);
        } finally {
            closeConnection(connection);
        }
    }

    private boolean addUserToGroup(Connection connection, RepoId repoId, GroupId groupId, UserId userId) throws CoreException {
        log.info("Adding user '{}' to group '{}' in repo '{}'", userId, groupId, repoId.id());

...
public class ApiUserController {
    ...
    public ResponseEntity<RestResponse<Void>> removeRole(@Parameter(description = "The repoId to search in.") @RequestParam("repoId") String repoId,
        ...
        try {
            userService.removeRole(RepoId.of(repoId), UserId.of(userId), role);
    ...
    public ResponseEntity<RestResponse<Void>> addRole(@Parameter(description = "The repoId to search in.") @RequestParam("repoId") String repoId,
                                                      ...
                                                      @Parameter(description = "The role to set.") @RequestParam("role") Role role) {
        try {
            userService.addRole(RepoId.of(repoId), UserId.of(userId), role);
            return RestResponse.<Void>success("Set role '%s' for '%s' in '%s'".formatted(role, userId, repoId), null).toResponse();
        } catch (ClientException e) {
            return RestResponse.<Void>error(e.getMessage()).toResponse();
        } catch (Exception e) {
    ...
    public ResponseEntity<RestResponse<Void>> addUser(@Parameter(description = "The repoId to search in. If none is given, returns the result for all currently loaded repos.") @RequestParam(value = "repoId") String repoId,
                                                      ...
                                                      @Parameter(description = "The users Role.") @RequestParam(value = "roles", required = false) Role role,
        ...
        try {
            ...
            userService.addUser(RepoId.of(repoId), new UserProfile(UserId.of(userId), password, new HashMap<>(), role == null ? Set.of() : Set.of(role), groupIdSet));

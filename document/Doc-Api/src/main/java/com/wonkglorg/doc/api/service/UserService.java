...
public class UserService{
	...
	public void createGroup(RepoId repoId, GroupId groupId, String groupName) throws InvalidRepoException, CoreException {
		repoService.validateRepoId(repoId);
		if(groupExists(repoId, groupId)){
			throw new CoreException("Group with id '%s' already exists in '%s'".formatted(groupId,repoId));
		}
		repoService.getRepo(repoId).getDatabase().createGroup(groupId, groupName);
	}
